      ...
      <div className="relative grid grid-cols-7 gap-px">
        ...
        return <div key={day.toString()} className={cn("aspect-square p-1 border border-border/10 relative group hover:bg-accent/5 transition-colors", isSameDay(day, new Date()) && "ring-1 ring-primary")}>
              ...
              <div className="absolute bottom-1 left-1 right-1">
                {dayEvents.slice(0, 2).map((event, i) => <div key={event.id} onClick={() => onEventClick(event.id)} className="h-1 w-full rounded-sm mb-0.5 bg-foreground/20 hover:bg-foreground/40 cursor-pointer transition-colors" title={event.title} />)}
