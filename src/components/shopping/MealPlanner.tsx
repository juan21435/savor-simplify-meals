...
const MealPlanner = () => {
  ...
  const getRecipesForDate = (date: Date) => {
    ...
    if (!mealPlan) return { breakfast: null, lunch: null, dinner: null };
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="md:col-span-1">
          <CardHeader>
            ...
            <CardDescription>Select a date to plan your meals</CardDescription>
          </CardHeader>
          <CardContent>
            <Calendar
              mode="single"
            ...
            <div className="mt-6">
              ...
              <ul className="space-y-2">
                {mealPlans.map((plan) => (
                  <li key={plan.id} className="flex justify-between p-2 hover:bg-muted rounded-md cursor-pointer"
                      onClick={() => setSelectedDate(new Date(plan.date))}>
              ...
              {weekRange.map((day) => {
                ...
                return (
                  <div key={day.toString()} 
                       className={`p-4 rounded-lg ${selectedDate && format(selectedDate, 'yyyy-MM-dd') === format(day, 'yyyy-MM-dd') 
                         ? 'border-2 border-primary' 
                         : 'border'}`}
                       onClick={() => setSelectedDate(day)}>
                    <div className="flex items-center justify-between mb-3">
                      <h3 className={`font-medium ${format(day, 'yyyy-MM-dd') === format(new Date(), 'yyyy-MM-dd') 
                        ? 'text-primary' 
                        : ''}`}>
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        <div className="space-y-2">
                          ...
                          {dayMeals.breakfast ? (
                            ...
                            <Button variant="outline" className="w-full h-auto py-2 border-dashed" size="sm">
                              <Plus className="h-3.5 w-3.5 mr-1" />
                              Add
                            </Button>
                          )}
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  ...
                  {getRecipesForDate(selectedDate).breakfast ? (
                    <RecipeCard recipe={getRecipesForDate(selectedDate).breakfast!} size="sm" />
                  ) : (
                ...
                <div>
                  ...
                  {getRecipesForDate(selectedDate).lunch ? (
                    <RecipeCard recipe={getRecipesForDate(selectedDate).lunch!} size="sm" />
                  ) : (
                    <div className="h-40 border-2 border-dashed rounded-lg flex items-center justify-center">
                      <Button variant="ghost">
                        <Plus className="h-5 w-5 mr-2" />
                ...
                <div>
                  ...
                  {getRecipesForDate(selectedDate).dinner ? (
                    <RecipeCard recipe={getRecipesForDate(selectedDate).dinner!} size="sm" />
                  ) : (
                    <div className="h-40 border-2 border-dashed rounded-lg flex items-center justify-center">
                      <Button variant="ghost">
                        <Plus className="h-5 w-5 mr-2" />
